# TODO ziqian adapt from fairseq/sequence_generator & fairseq/examples/speech_to_speech/unity/
# to implement sequential decoding
...
def _get_sep_info(item, sep_idx = 4):
    sep_indice = torch.cat([ torch.tensor([0]).to(item.device),torch.nonzero(item == sep_idx).view(-1)]).long()
    return sep_indice

def get_sep_info(dataset, pad_idx = -2):
    # get the indice of <sep> in a given dataset/ batch
    # TODO ziqian, fix the pad_idx of <sep> indice somewhere, not just specify it everywhere
...
def get_nb_sents(sep_info):
    # get the number of sentence in each document
    return  torch.LongTensor(list(map(lambda x: len(x), sep_info)))



# TODO ziqian maybe change a better name
class SequenceGeneratorDoc(SequenceGenerator):
    # todo fix self.search as search.BeamSearch ?
    def __init__(
        self,
        models,
        tgt_dict,
        ...
        """Generates translations of a given source document.
        decode sentence by sentence with the beam search, 
        also update the context_mask once a sentence is finalized

        check more argument description at fairseq.SequenceGenerator
        ...
        if isinstance(models, EnsembleModelMask):
            self.model = models
        else:
            self.model = EnsembleModelMask(models)
    ...
    def _generate(
        ...
        # print('TEST.generator (valid) bsz src_len', bsz, src_len )
        ...
        with torch.autograd.profiler.record_function("EnsembleModelMask: forward_encoder"):
            # encoder.forward don't accept argument about src_sep_idx
            encoder_input = {k : net_input[k] for k in ['src_tokens', 'src_lengths']}
        ...
        #########  <sep> indice 
        # sep_indice_x of shape (bsz, max_nb_sents)
        sep_indice_src =  net_input.get('sep_idx_src', torch.zeros(bsz,1).long()).to(src_tokens.device)
        # sep_indice_src = sep_indice_src.repeat_interleave( beam_size, dim = 0)
        sep_indice_src = sep_indice_src.index_select(0, new_order).to(src_tokens.device)
        # target
        sep_indice_tgt = torch.zeros(bsz,1).long().to(src_tokens.device)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                #### update sep indices for active hypos
                sep_indice_src = (sep_indice_src).index_select(0, reorder_state)
                ####
            
            with torch.autograd.profiler.record_function(
                "EnsembleModelMask: forward_decoder"
            ):
            ...
            if (
                ...
                print('-------------------- prefix tokens is not None')
            ...
            # update sep indice matrix if <sep> is generated at current step
            # do nothing if there is only one segment in source
            if sep_indice_src.size()[-1] > 1 :
                # detect if <sep> tag is generated at this step
                # TODO ziqian (not urgent)  figure out why sep_bbsz_idx not corresponds to sep idx in token[:,step+1]
                # # Shape of sep_mask: (batch size, beam size)
                # sep_mask = cand_indices.eq(self.sep) & cand_scores.ne(-math.inf)
                # sep_mask[:, :beam_size][cands_to_ignore] = torch.tensor(0).to(sep_mask)
                # # Shape: 1d list of absolute-numbered
...
class EnsembleModelMask(EnsembleModel):
    """
    A wrapper around an ensemble of models.

    rewrote forward_decoder to allow sep_indice of source and target as argument
    """
    @torch.jit.export
    def forward_decoder(
        self,
