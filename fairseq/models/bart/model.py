...
class BARTModel(TransformerModel):
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if self.encoder.embed_positions.weight.shape[0] > loaded_num_pos and self.encoder.embed_positions.weight.shape[1] == loaded_dim_pos: 
            logger.info( 
                f"extending encoder positional embedding size of loaded checkpoint from ({loaded_num_pos}, {loaded_dim_pos}) to {self.encoder.embed_positions.weight.shape}"
            )
            extend_embed_pos_enc = self.encoder.embed_positions.weight.detach().clone()
            extend_embed_pos_enc[:loaded_num_pos] = state_dict['encoder.embed_positions.weight']
            state_dict['encoder.embed_positions.weight'] = extend_embed_pos_enc


        loaded_num_pos, loaded_dim_pos = state_dict['decoder.embed_positions.weight'].shape
        if self.decoder.embed_positions.weight.shape[0] > loaded_num_pos and self.decoder.embed_positions.weight.shape[1] == loaded_dim_pos: 
            logger.info( 
                f"extending decoder positional embedding size of loaded checkpoint from ({loaded_num_pos}, {loaded_dim_pos}) to {self.decoder.embed_positions.weight.shape}"
            )

            extend_embed_pos_dec = self.decoder.embed_positions.weight.detach().clone()
