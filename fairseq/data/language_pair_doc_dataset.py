...
#         sep_info.append(sep_indice)
#         nb_sents.append(len(sep_indice))
#     return sep_info, torch.LongTensor(nb_sents)
...
def collate(
    ...
    def merge(key, left_pad, pad_idx = pad_idx, move_eos_to_beginning=False, pad_to_length=None):
    ...
    # in criterion, model is called by `model(**samples['net_input'])`: 'net_input' contains the input arguments
    batch = {
        "id": id,
...
class LanguagePairDocDataset(LanguagePairDataset):
    """
    ...
    def __init__(
            ...
            self.tgt_nb_sents = None
            self.tgt_sep_indice = None
            if tgt is not None: 
                sep_info, nb_sents = doc_size_in_sents(tgt)
                self.tgt_nb_sents = nb_sents
                self.tgt_sep_indice = sep_info
                assert len(self.src_nb_sents) == len(self.src_sizes)
                assert len(self.src_nb_sents) == len(self.tgt_nb_sents)
    ...
    def ordered_indices(self):
        ...
        if self.buckets is None:
            ...
            if self.tgt_sizes is not None:
                tgt_stat = np.vstack(
                    (self.tgt_nb_sents.cpu().numpy()[indices],  self.tgt_sizes[indices])
                    ).T.tolist() 
                # sort by target length for document with the same number of sentences
                indices = indices[
                    sorted(range(len(tgt_stat)), key=lambda k: (tgt_stat[k][0], tgt_stat[k][1]))
                ]
                # indices = indices[np.argsort(self.tgt_sizes[indices], kind="mergesort")]
            src_stat = np.vstack(
                (self.src_nb_sents.cpu().numpy()[indices],  self.src_sizes[indices])
                ).T.tolist()  
            return  indices[sorted(range(len(src_stat)), key=lambda k: (src_stat[k][0], src_stat[k][1]))]
        else:
            # sort by bucketed_num_tokens, which is:
            #   max(padded_src_len, padded_tgt_len)
            bucket_stat = np.vstack(
                (self.src_nb_sents.cpu().numpy()[indices],  self.bucketed_num_tokens[indices])
                ).T.tolist() 
            return  indices[sorted(range(len(bucket_stat)), key=lambda k: (bucket_stat[k][0], bucket_stat[k][1]))]
